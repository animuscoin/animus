...
###   animuscoin.io and animus.org and compares them.
...
WORKINGDIR="/tmp/animus_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://animuscoin.io"
HOST2="https://animus.org"
BASEDIR="/bin/"
VERSIONPREFIX="animus-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'animus-',
...
if [ $? -ne 0 ]; then
   echo "animus.org failed to provide signature file, but animuscoin.io did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "animus.org and animuscoin.io signature files were not equal?"
