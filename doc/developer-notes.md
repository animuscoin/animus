...
- [Development guidelines](#development-guidelines)
    - [General Bitcoin Core](#general-animus-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play animuss" on the test network, if you
...
([`valgrind.supp`](https://github.com/animuscoin/animus/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_animus
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_animus --log_level=test_suite
$ valgrind -v --leak-check=full src/animusd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_animus.coverage/index.html`.
```
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/animuscoin/animus/issues/12691)
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `ANS_FOO_BAR_H`, e.g.

```c++
#ifndef ANS_FOO_BAR_H
#define ANS_FOO_BAR_H
...
#endif // ANS_FOO_BAR_H
```
...
- src/libsecp256k1
  - Upstream at https://github.com/animus-core/secp256k1/ ; actively maintaned by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/animus-core/ctaes ; actively maintained by Core contributors.
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:animus/animus.git
...
Commit [`bb81e173`](https://github.com/animuscoin/animus/commit/bb81e173) is an example of a scripted-diff.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `animus-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `animus-cli` and the GUI debug console use this table to determine how to
