...
example /usr/src/animus, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\animus`, for example:

    make install DESTDIR=/mnt/c/workspace/animus
...
causes two of the animus executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the animus code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the animus source code).
