...
const char * const BITCOIN_CONF_FILENAME = "animus.conf";
const char * const BITCOIN_PID_FILENAME = "animusd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "animus" is a composite category enabling all Animus-related debug output
            if(ptrCategory->count(string("animus"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AnimusCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AnimusCore
    // Mac: ~/Library/Application Support/AnimusCore
    // Unix: ~/.animuscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AnimusCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AnimusCore";
#else
    // Unix
    return pathRet / ".animuscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty animus.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override animus.conf
